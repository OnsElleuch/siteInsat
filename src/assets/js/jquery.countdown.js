 ...
 $.countdown.UTCDate(-7, new Date(2013, 12-1, 25, 12, 0)) */
    ...
    _generateHTML: function (inst) {
      ...
      var showCompact = function (period) {
        ...
        return show[period]
          ? self._translateDigits(inst, inst._periods[period]) + (labelsNum ? labelsNum[period] : labels[period]) + ' '
          : '';
      };
      ...
      return inst.options.layout
        ...
        : (inst.options.compact // Compact version
            ? '<span class="' +
              ...
              (show[M]
                ? (show[H] ? inst.options.timeSeparator : '') + this._minDigits(inst, inst._periods[M], 2)
                : '') +
              (show[S]
                ? (show[H] || show[M] ? inst.options.timeSeparator : '') + this._minDigits(inst, inst._periods[S], 2)
                : '')
            : // Full version
              '<span class="' +
              this._rowClass +
            ...
            (inst.options.description
              ? '<span class="' + this._rowClass + ' ' + this._descrClass + '">' + inst.options.description + '</span>'
              : '');
    },
    ...
    _buildLayout: function (inst, show, layout, compact, significant, showSignificant) {
      ...
      var labelFor = function (index) {
        return (inst.options[(compact ? 'compactLabels' : 'labels') + whichLabels(inst._periods[index])] || labels)[
          index
        ];
      };
    ...
    _calculatePeriods: function (inst, show, significant, now) {
      ...
      if (show[Y] || show[O]) {
        ...
        var sameDay =
          ...
          (until.getDate() >= Math.min(lastNow, lastUntil) && now.getDate() >= Math.min(lastNow, lastUntil));
